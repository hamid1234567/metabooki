...
export function TabsInteractiveV3({ block }: { block: InteractiveV3Block }) {
  ...
  const dir = directionFromText([title, ...tabs.map(directionTextFromItem)].filter(Boolean).join(' '))
  if (!tabs.length) return null
  return (
    <section className="interactive-v3 interactive-v3-tabs" dir={dir} style={{ '--tab-accent': accent } as CSSProperties}>
      {title && <h3>{title}</h3>}
