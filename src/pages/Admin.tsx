              ...
              {aiSettings.providers.map(provider => (
                <div key={provider.id} className={`rounded-2xl border p-4 ${aiSettings.activeProvider === provider.id ? 'border-primary bg-primary/5' : 'border-border bg-background/50'}`}>
                  ...
                  <div className="space-y-3">
                    ...
                    <input title="مدل متن و تصویر ورودی" value={provider.model} onChange={e => updateAiProvider(provider.id, { model: e.target.value })} placeholder="Text/Vision model مثل gpt-4o-mini" className="w-full p-2.5 rounded-xl border border-input bg-background text-sm" dir="ltr" />
                    <input title="مدل تولید تصویر" value={provider.imageModel || ''} onChange={e => updateAiProvider(provider.id, { imageModel: e.target.value })} placeholder="Image generation model مثل gpt-image-1" className="w-full p-2.5 rounded-xl border border-input bg-background text-sm" dir="ltr" />
                    <p className="text-xs text-muted-foreground leading-6">مدل اول برای متن و تحلیل تصویر است؛ مدل دوم فقط برای تولید تصویر استفاده می‌شود.</p>
