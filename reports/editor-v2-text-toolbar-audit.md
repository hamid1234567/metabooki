...
- تکه‌های پشت‌سرهم inline بعد از خروج از list قبل از ذخیره در یک پاراگراف واحد ادغام می‌شوند؛ بنابراین لینک‌ها، markها و styleهای inline داخل همان متن حفظ می‌شوند.
- ابزار رنگ متن از color picker آزاد به ۶ سواچ ثابت وب تبدیل شد؛ refresh سند بعد از کلیک‌های toolbar بدون جایگزینی سطح ادیتور انجام می‌شود تا selection بعد از رنگ اول از بین نرود و رنگ‌های بعدی هم اعمال شوند.
...
| Text color | پاس کدی | پاس کدی | پاس کدی | رنگ از ۶ سواچ ثابت اعمال می‌شود؛ هر کلیک، color جدید را مستقیم روی selection یا بلاک انتخاب‌شده ذخیره می‌کند و DOM ادیتور را همان لحظه بازسازی نمی‌کند. |
...
| Bullet list | پاس کدی | پاس کدی | پاس کدی | `<ul><li>` به block نوع `list` تبدیل می‌شود و خروج از list به پاراگراف واحد با لینک و استایل inline حفظ‌شده تبدیل می‌شود. |
| Numbered list | پاس کدی | پاس کدی | پاس کدی | `<ol><li>` به block نوع `list` تبدیل می‌شود و خروج از number به پاراگراف واحد با لینک و استایل inline حفظ‌شده تبدیل می‌شود. |
